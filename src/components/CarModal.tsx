...
const CarModal: React.FC<CarModalProps> = ({ open, initialCar, onSave, onDelete, onClose, isEdit }) => {
  ...
  useEffect(() => {
    ...
    setError('');
    setBootShapeMode((initialCar?.bootShapeMode as any) || (initialCar?.bootMesh ? 'mesh' : 'cube'));
    if (initialCar?.bootShapeMode === 'mesh' && (initialCar as any)?.bootMesh?.slabs) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (bootShapeMode === 'mesh') {
      const slabs = (meshSlabs.length ? meshSlabs : [makeDefaultSlab()]).map((s) => ({
        ...
        depth: Math.max(0.1, Math.min(s.depth, Math.max(0.1, D - Math.max(0, Math.min(s.zStart, D))))),
    ...
    onSave({ name, W, D, H_shelf_in: finalHIn, H_shelf_out: finalHOut, ...(extra as any) }, initialCar?.key);
  };
      ...
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="input-group">
            <label>Boot shape
              <select value={bootShapeMode} onChange={(e) => setBootShapeMode(e.target.value as any)}>
          ...
          {bootShapeMode === 'mesh' && (
            <div style={{ borderTop: '1px solid #2a2f3a', marginTop: 12, paddingTop: 12 }}>
              <div style={{ marginBottom: 8, fontWeight: 600 }}>Mesh slabs (symmetric trapezoids)</div>
              {meshSlabs.length === 0 && (
                <div className="badge" style={{ marginBottom: 8 }}>No slabs yet. A default slab will be created if left empty.</div>
              )}
              {meshSlabs.map((s, i) => (
                <div key={i} className="input-grid" style={{ alignItems: 'end', marginBottom: 8 }}>
                  <div className="input-group"><label>y<input type="number" value={s.y} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, y: Number(e.target.value) } : sv))} /></label></div>
                  <div className="input-group"><label>height<input type="number" value={s.height} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, height: Number(e.target.value) } : sv))} /></label></div>
                  <div className="input-group"><label>zStart<input type="number" value={s.zStart} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, zStart: Number(e.target.value) } : sv))} /></label></div>
                  <div className="input-group"><label>depth<input type="number" value={s.depth} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, depth: Number(e.target.value) } : sv))} /></label></div>
                  <div className="input-group"><label>backHalfW<input type="number" value={s.backHalfW} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, backHalfW: Number(e.target.value) } : sv))} /></label></div>
                  <div className="input-group"><label>frontHalfW<input type="number" value={s.frontHalfW} onChange={(e) => setMeshSlabs(v => v.map((sv, idx) => idx === i ? { ...sv, frontHalfW: Number(e.target.value) } : sv))} /></label></div>
                  <div className="button-group" style={{ marginTop: 0 }}>
                    <button type="button" className="btn danger" onClick={() => setMeshSlabs(v => v.filter((_, idx) => idx !== i))}>Remove</button>
                  </div>
                </div>
              ))}
              <div className="button-group">
                <button type="button" className="btn" onClick={() => setMeshSlabs(v => [...v, makeDefaultSlab()])}>+ Add slab</button>
              </div>
            </div>
          )}
